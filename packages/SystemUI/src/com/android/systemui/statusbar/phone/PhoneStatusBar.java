...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode {
	...
	private ContentObserver mUserSetupObserver = new ContentObserver(
		...
		public void onChange(boolean selfChange) {
			final boolean userSetup = 0 != Settings.Secure.getIntForUser(
					...
					mCurrentUserId);
			if (MULTIUSER_DEBUG)
				Log.d(TAG, String.format("User setup changed: "
	...
	final private ContentObserver mHeadsUpObserver = new ContentObserver(
			mHandler) {
		@Override
		public void onChange(boolean selfChange) {
	...
	protected PhoneStatusBarView makeStatusBarView() {
		...
		if (ENABLE_HEADS_UP) {
			mHeadsUpNotificationView = (HeadsUpNotificationView) View.inflate(
					context, R.layout.heads_up, null);
		...
		if (DEBUG)
			Log.v(TAG, "carrierlabel=" + mCarrierLabel + " show="
					+ mShowCarrierInPanel);
	...
	private void prepareNavigationBarView() {
		...
		mNavigationBarView.getRecentsButton().setOnTouchListener(
	...
	public void addIcon(String slot, int index, int viewIndex,
			StatusBarIcon icon) {
		if (SPEW)
			Log.d(TAG, "addIcon slot=" + slot + " index=" + index
	...
	public void updateIcon(String slot, int index, int viewIndex,
		...
		if (SPEW)
			Log.d(TAG, "updateIcon slot=" + slot + " index=" + index
	...
	public void removeIcon(String slot, int index, int viewIndex) {
		if (SPEW)
			Log.d(TAG, "removeIcon slot=" + slot + " index=" + index
					+ " viewIndex=" + viewIndex);
	...
	public void addNotification(IBinder key, StatusBarNotification notification) {
		if (DEBUG)
			Log.d(TAG, "addNotification score=" + notification.getScore());
	...
	protected void refreshLayout(int layoutDirection) {
		...
		if (mClearButton != null && mClearButton instanceof ImageView) {
			...
			((ImageView) mClearButton)
					.setImageResource(R.drawable.ic_notify_clear);
		}
	...
	protected void updateNotificationIcons() {
		...
		for (int i = 0; i < N; i++) {
			Entry ent = mNotificationData.get(N - i - 1);
			if (!((provisioned && ent.notification.getScore() >= HIDE_ICONS_BELOW_SCORE) || showNotificationEvenIfUnprovisioned(ent.notification)))
				continue;
			if (!notificationIsForCurrentUser(ent.notification))
				continue;
			toShow.add(ent.icon);
	...
	protected void updateCarrierLabelVisibility(boolean force) {
		...
		if (force || mCarrierLabelVisible != makeVisible) {
			...
			if (DEBUG) {
				Log.d(TAG, "making carrier label "
	...
	void makeExpandedVisible() {
		if (SPEW)
			Log.d(TAG, "Make expanded visible: expanded visible="
					+ mExpandedVisible);
	...
	public void animateCollapsePanels(int flags) {
		if (SPEW) {
			Log.d(TAG, "animateCollapse():" + " mExpandedVisible="
					+ mExpandedVisible + " flags=" + flags);
		}
	...
	void makeExpandedInvisible() {
		if (SPEW)
			Log.d(TAG, "makeExpandedInvisible: mExpandedVisible="
		...
		if (DEBUG_GESTURES) {
			if (event.getActionMasked() != MotionEvent.ACTION_MOVE) {
				EventLog.writeEvent(EventLogTags.SYSUI_STATUSBAR_TOUCH,
						event.getActionMasked(), (int) event.getX(),
						(int) event.getY(), mDisabled);
			}

		}

		if (SPEW) {
			Log.d(TAG, "Touch: rawY=" + event.getRawY() + " event=" + event
	...
	public void setSystemUiVisibility(int vis, int mask) {
		...
		if (DEBUG)
			Log.d(TAG,
					String.format(
							...
							Integer.toHexString(vis),
							Integer.toHexString(mask),
							Integer.toHexString(oldVal),
							Integer.toHexString(newVal),
		...
		if (diff != 0) {
			...
			final int sbMode = computeBarMode(oldVal, newVal,
					mStatusBarView.getBarTransitions(),
					View.STATUS_BAR_TRANSIENT, View.STATUS_BAR_TRANSLUCENT);

			// update navigation bar mode
			final int nbMode = mNavigationBarView == null ? -1
					: computeBarMode(oldVal, newVal,
	...
	private int computeBarMode(int oldVis, int newVis,
			BarTransitions transitions, int transientFlag, int translucentFlag) {
	...
	private void checkBarModes() {
		...
		if (panelsEnabled()
			...
			// if panels are expandable, force the status bar opaque on any
			// interaction
			sbMode = MODE_OPAQUE;
		}
		checkBarMode(sbMode, mStatusBarWindowState,
				mStatusBarView.getBarTransitions());
		if (mNavigationBarView != null) {
			checkBarMode(mNavigationBarMode, mNavigationBarWindowState,
					mNavigationBarView.getBarTransitions());
		}
	}
	...
	private void checkBarMode(int mode, int windowState,
		...
		final boolean anim = (mScreenOn == null || mScreenOn)
	...
	public void setInteracting(int barWindow, boolean interacting) {
		mInteractingWindows = interacting ? (mInteractingWindows | barWindow)
	...
	public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
		...
		dumpBarTransitions(pw, "mStatusBarView",
				mStatusBarView.getBarTransitions());
		if (mNavigationBarView != null) {
		...
		if (mNotificationPanel != null) {
			pw.println("    mNotificationPanel=" + mNotificationPanel
					+ " params="
		...
		if (mSettingsPanel != null) {
			pw.println("    mSettingsPanel=" + mSettingsPanel + " params="
					+ mSettingsPanel.getLayoutParams().debug(""));
			pw.print("      ");
		...
		if (DUMPTRUCK) {
			synchronized (mNotificationData) {
				...
				for (int i = 0; i < N; i++) {
					...
					pw.println("         pkg=" + n.getPackageName() + " id="
							+ n.getId() + " score=" + n.getScore());
			...
			if (false) {
				...
				mHandler.post(new Runnable() {
					public void run() {
						...
						Log.d(TAG, "mStatusBarView: ----- (" + mAbsPos[0] + ","
								+ mAbsPos[1] + ") " + mStatusBarView.getWidth()
	...
	private static void dumpBarTransitions(PrintWriter pw, String var,
			BarTransitions transitions) {
		pw.print("  ");
		pw.print(var);
	...
	public void updateExpandedViewPos(int thingy) {
		if (SPEW)
			Log.v(TAG, "updateExpandedViewPos");
			...
			synchronized (mNotificationData) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						final int velocity = dir * width * 8; // 1000/8 = 125 ms
																// duration
	...
	private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(action)) {
				...
				if (bundle != null) {
					String command = bundle.getString("command", "").trim()
							.toLowerCase();
					if (command.length() > 0) {
						try {
							dispatchDemoCommand(command, bundle);
						} catch (Throwable t) {
							Log.w(TAG, "Error running demo command, intent="
									+ intent, t);
						}
					}
				}
	...
	private void setHeadsUpVisibility(boolean vis) {
		...
		if (!vis) {
			if (DEBUG)
				Log.d(TAG, "setting heads up entry to null");
	...
	protected void loadDimens() {
		...
		if (newIconHPadding != mIconHPadding || newIconSize != mIconSize) {
			// Log.d(TAG, "size=" + newIconSize + " padding=" +
			// newIconHPadding);
		...
		mHeadsUpNotificationDecay = res
				.getInteger(R.integer.heads_up_notification_decay);
		mRowHeight = res
	...
	public void dispatchDemoCommand(String command, Bundle args) {
		if (!mDemoModeAllowed) {
			mDemoModeAllowed = Settings.Global.getInt(
					mContext.getContentResolver(), "sysui_demo_allowed", 0) != 0;
		}
		if (!mDemoModeAllowed)
			return;
		...
		boolean modeChange = command.equals(COMMAND_ENTER)
				|| command.equals(COMMAND_EXIT);
		...
		if (mNetworkController != null
				&& (modeChange || command.equals(COMMAND_NETWORK))) {
			mNetworkController.dispatchDemoCommand(command, args);
		}
		if (command.equals(COMMAND_BARS)) {
			String mode = args.getString("mode");
			int barMode = "opaque".equals(mode) ? MODE_OPAQUE : "translucent"
					.equals(mode) ? MODE_TRANSLUCENT : "semi-transparent"
					.equals(mode) ? MODE_SEMI_TRANSPARENT : -1;
			if (barMode != -1) {
				boolean animate = true;
				if (mStatusBarView != null) {
					mStatusBarView.getBarTransitions().transitionTo(barMode,
							animate);
				}
				if (mNavigationBarView != null) {
					mNavigationBarView.getBarTransitions().transitionTo(
							barMode, animate);
				}
			}
		}
